module satay::vault {
    ...
    struct Vaults has key {
        vault_accounts: Table<String, SignerCapability>,
    }
    ...
    public fun initialize(satay_admin: &signer) {
        move_to(satay_admin, Vaults { vault_accounts: table::new() });
    }

    public fun create_vault(manager: &signer, seed: vector<u8>) {
        let (resource_signer, signer_capability) = account::create_resource_account(manager, seed);
        coins::register_internal<USDC>()
    }
