#[test_only]
module satay_ditto_farming::mock_ditto_farming_strategy {
    ...
    public entry fun harvest(
        ...
        let (
            ditto_farming_coin,
            residual_aptos_coin
        ) = mock_ditto_farming::reinvest_returns(keeper);
        ...
        if(debt_payment_amount + profit_amount > 0) {
            let lp_to_liquidate = mock_ditto_farming::get_farming_coin_amount_for_apt_amount(
    ...
    public entry fun withdraw_for_user(
        ...
        if(debt_payment_amount > 0){
            let lp_to_burn = mock_ditto_farming::get_farming_coin_amount_for_apt_amount(debt_payment_amount);
            let strategy_coins = base_strategy::withdraw_strategy_coin<DittoStrategy, DittoFarmingCoin>(
                &vault_cap,
                lp_to_burn,
