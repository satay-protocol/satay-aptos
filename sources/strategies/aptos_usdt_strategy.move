module satay::aptos_usdt_strategy {
    ...
    public fun run_strategy(_acc: &signer, manager_addr: address, vault_id: u64) {
        ...
        let aptos_coins = vault::fetch_pending_coins(&vault_cap);
    ...
    public fun liquidate_position(_acc: &signer, _manager_addr : address, _vault_id : u64, _amount : u64) {
        // let (vault_cap, lock) = satay::lock_vault<AptosUsdcLpStrategy>(
        //     manager_addr,
        //     vault_id,
        //     AptosUsdcLpStrategy {}
        ...
        //
        // vault::deposit<USDT>(&vault_cap, usdt_coins);
        //
        // satay::unlock_vault<AptosUsdcLpStrategy>(manager_addr, vault_cap, lock);
    }
    ...
    fun swap<From, To>(coins: Coin<From>): Coin<To> {
        // swap on AMM
        router::swap_exact_coin_for_coin<From, To, LP<USDT, AptosCoin, Uncorrelated>>(
            coins,
            0
        )
    }

    fun add_liquidity(aptos_coins : Coin<AptosCoin>, usdt_coins : Coin<USDT>) : (Coin<USDT>, Coin<AptosCoin>, Coin<LP<USDT, AptosCoin, Uncorrelated>>) {
        router::add_liquidity(usdt_coins, 1, aptos_coins, 1)
    }

    fun remove_liquidity(lp_coins : Coin<LP<AptosCoin, USDT, Uncorrelated>>) : (Coin<AptosCoin>, Coin<USDT>) {
        router::remove_liquidity<AptosCoin, USDT, Uncorrelated>(lp_coins, 1, 1)
    }
}
