...
module satay::strategy_config {
    ...
    /// holds the strategy manager information for each (StrategyType, BaseCoin), stored in strategy account
    ...
    struct StrategyConfig<phantom StrategyType: drop, phantom BaseCoin> has key {
    ...
    public(friend) fun initialize<StrategyType: drop, BaseCoin>(
        ...
        move_to(strategy_account, StrategyConfig<StrategyType, BaseCoin> {
    ...
    public entry fun set_strategy_manager<StrategyType: drop, BaseCoin>(
        strategy_manager: &signer,
        strategy_address: address,
        new_strategy_manager_address: address
    )
    acquires StrategyConfig {
        assert_strategy_config_exists<StrategyType, BaseCoin>(strategy_address);
        assert_strategy_manager<StrategyType, BaseCoin>(strategy_manager, strategy_address);
        let strategy_config = borrow_global_mut<StrategyConfig<StrategyType, BaseCoin>>(strategy_address);
    ...
    public entry fun accept_strategy_manager<StrategyType: drop, BaseCoin>(
    ...
    /// @param strategy_address - the address of the strategy
    public fun get_strategy_manager_address<StrategyType: drop, BaseCoin>(strategy_address: address): address
    acquires StrategyConfig {
        assert_strategy_config_exists<StrategyType, BaseCoin>(strategy_address);
        let config = borrow_global<StrategyConfig<StrategyType, BaseCoin>>(strategy_address);
    ...
    /// @param strategy_address - the address of the strategy
    public fun assert_strategy_manager<StrategyType: drop, BaseCoin>(
        strategy_manager: &signer,
        strategy_address: address
    )
    acquires StrategyConfig {
    ...
    /// asserts that StrategyCoinConfig<StrategyType, BaseCoin> exists on strategy_address
    /// @param strategy_address - the address of the strategy
    fun assert_strategy_config_exists<StrategyType: drop, BaseCoin>(strategy_address: address) {
        assert!(exists<StrategyConfig<StrategyType, BaseCoin>>(strategy_address), ERR_CONFIG_DOES_NOT_EXIST);
    }
}
