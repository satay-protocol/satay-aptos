...
module satay::keeper_config {
    ...
    struct KeeperConfig<phantom StrategyType, phantom BaseCoin> has key {
    ...
    public(friend) fun initialize<StrategyType: drop, BaseCoin>(vault_account: &signer, _witness: &StrategyType) {
        move_to(vault_account, KeeperConfig<StrategyType, BaseCoin> {
    ...
    acquires KeeperConfig {
        assert_strategy_config_exists<StrategyType, BaseCoin>(vault_address);
        vault_config::assert_vault_manager(vault_manager, vault_address);
        let strategy_config = borrow_global_mut<KeeperConfig<StrategyType, BaseCoin>>(vault_address);
    ...
    public entry fun accept_keeper<StrategyType: drop, BaseCoin>(new_keeper: &signer, vault_address: address)
    acquires KeeperConfig {
        assert_strategy_config_exists<StrategyType, BaseCoin>(vault_address);
        let vault_config = borrow_global_mut<KeeperConfig<StrategyType, BaseCoin>>(vault_address);
    ...
    /// @param vault_address - the address of the vault holding KeeperConfig<StrategyType>
    public fun get_keeper_address<StrategyType: drop, BaseCoin>(vault_address: address): address
    acquires KeeperConfig {
        assert_strategy_config_exists<StrategyType, BaseCoin>(vault_address);
        let config = borrow_global<KeeperConfig<StrategyType, BaseCoin>>(vault_address);
    ...
    /// @param vault_address - the address of the vault holding KeeperConfig<StrategyType>
    public fun assert_keeper<StrategyType: drop, BaseCoin>(keeper: &signer, vault_address: address)
    acquires KeeperConfig {
        assert_strategy_config_exists<StrategyType, BaseCoin>(vault_address);
        let config = borrow_global<KeeperConfig<StrategyType, BaseCoin>>(vault_address);
    ...
    fun assert_strategy_config_exists<StrategyType: drop, BaseCoin>(vault_address: address) {
        assert!(exists<KeeperConfig<StrategyType, BaseCoin>>(vault_address), ERR_CONFIG_DOES_NOT_EXIST);
    }
}
